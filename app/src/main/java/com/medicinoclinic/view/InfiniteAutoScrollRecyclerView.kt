...
import com.medicinoclinic.adapter.InfiniteAutoScrollAdapter
...
const val DEFAULT_SCROLL_SPEED = 3000f
...
//        }
    ...
    private fun getEvenLayoutRes(scrollLayoutType: Int, scrollOrientation: Int) =
        if (scrollLayoutType == ScrollLayoutType.LIST && scrollOrientation == VERTICAL) {
            R.layout.item_infinite_scroll_list_news
        } else if (scrollLayoutType == ScrollLayoutType.GRID && scrollOrientation == VERTICAL) {
            R.layout.item_infinite_scroll_list_news
        } else {
            R.layout.item_infinite_scroll_list_news
        }

    fun startScrolling(contents: List<ContentModel>) {
